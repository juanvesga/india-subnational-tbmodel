...
prm<-input$prm ; ref<-input$ref; 
prm$itv <-itv; 
sel<-input$sel  ; agg<-input$agg;
gps<-input$gps
...
if(strcmp(action,"simplex")){
  ...
  x1 <-fminsearch(fun=obj_spx, x0=x, options = opt)
  x  <-x1$optbase$xopt
  #  
...
if(strcmp(action,"sim_intv")){
  ...
  if (file.exists(f)){
    ...
    runs<-readRDS(f)
    x1<-readRDS(ff)
    itvs<-sim_pack_intervention(x1,runs$sfin,prm,ref,sel,agg,gps,location,c())
    plot_sim_intv(itvs)
